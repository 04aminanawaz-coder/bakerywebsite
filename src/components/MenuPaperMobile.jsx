...
// Small-screen version of MenuPaper: same paper look, scaled down to fit
// phones and tablets. No chef animation trigger (no chef is shown there).
export default function MenuPaperMobile() {
  const [showScrollHint, setShowScrollHint] = useState(true);

  return (
    <div
      className="relative w-full max-w-[600px] h-[75vh] min-h-[480px] max-h-[650px] bg-no-repeat bg-center"
      style={{
        ...
        {menuSections.map((section) => (
          <div key={section.title} className="mb-10 sm:mb-14">
            ...
            {section.items.map((item) => (
              <div
                key={item.name}
                className="flex items-center justify-between mb-5 sm:mb-7 rounded-xl p-2 sm:p-3"
              >
